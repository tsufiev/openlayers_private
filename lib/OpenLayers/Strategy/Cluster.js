...
OpenLayers.Strategy.Cluster = OpenLayers.Class(OpenLayers.Strategy, {
    ...
    initQuad: function(features) {
        if ( this.quadTree )
            this.quadTree.clear();
        var quadTree = new QuadTree(
            this.layer.getDataExtent(features), 
            true, 30, 4, function(feature) {
    ...
    cluster: function(event) {
        if((!event || event.zoomChanged) && this.features) {
            ...
            if(resolution != this.resolution || !this.clustersExist()) {
                ...
                var clusters = [];
                var feature, cluster, i, j, len;

                var quadTree= this.initQuad(this.features);
                for(i=0; i<this.features.length; ++i) {
                    feature = this.features[i];
                    if ( feature.geometry && !feature.clustered ) {
                        cluster = this.createCluster(feature);
                        var center = cluster.geometry,
                            radius = this.distance * this.resolution,
                            bbox = new OpenLayers.Bounds(
                                center.x-radius, center.y-radius,
                                center.x+radius, center.y+radius),
                            features = quadTree.retrieveBBox(bbox);

                        for (len = features.length, j = 0; j < len; j++)
                            this.addToCluster(cluster, features[j]);
    ...
    shouldCluster: function(cluster, feature) {
        function f(x) {
            return x.geometry.getBounds().getCenterLonLat();
        }
        var cc = f(cluster),
            fc = f(feature),
            distance = (
